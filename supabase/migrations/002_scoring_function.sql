-- ============================================================
-- SCORING — Fonction de calcul des points à la révélation
-- Appelée par l'Edge Function au moment du reveal
-- ============================================================
...
begin
  ...
  loop
    ...
    -- 2. ARÔMES (100 pts × nb joueurs ayant mis le même arôme)
    --    + 50 pts bonus si l'arôme est dans la liste officielle
    v_pts_aromes := 0;
    for v_arome in select jsonb_array_elements_text(v_tasting.aromes)
    loop
      -- Compter combien de joueurs ont mis cet arôme
      select count(*) into v_arome_count
      from public.tastings
      where session_id = p_session_id
        and aromes @> to_jsonb(v_arome::text);

      -- Points = (nb joueurs qui l'ont mis - 1) × 100
