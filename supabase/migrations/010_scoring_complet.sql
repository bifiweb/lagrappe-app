-- ============================================================
-- SCORING COMPLET — Migration consolidée (remplace 005→008)
-- À appliquer si les migrations 005-008 n'ont pas encore été
-- exécutées dans Supabase.
-- ============================================================

-- Colonnes manquantes éventuelles
alter table public.tastings
  add column if not exists pts_elevage int not null default 0;

alter table public.session_players
  add column if not exists avatar text;

-- Barème final :
-- Robe      : 300 si juste · 100 sinon
-- Arômes    : 300 × nb joueurs même arôme + 300 si officiel
-- Bouche    : 300 si juste · 100 sinon
-- Prix      : 500 si exact · −50 pts/CHF d'écart (min 0)
-- Millésime : 500 si juste · 100 sinon
-- Cépage    : 1 000 si juste · 200 sinon
-- Région    : 500 si juste · 100 sinon
-- Élevage   : 300 si juste · 100 sinon
-- Aides     : −200 pts par aide utilisée
...
begin
  ...
  loop
    ...
    -- 4. PRIX (500 si exact · −50 pts/CHF d'écart · min 0)
    ...
    if v_tasting.millesime_estime = v_notes.millesime then
      update public.tastings set pts_millesime = 500 where id = v_tasting.id;
      v_total := v_total + 500;
    else
    ...
    -- 9. MALUS AIDES (−200 pts par aide · min 0 au total)
